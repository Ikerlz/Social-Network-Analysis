...
class MovieLens(object):
    def __init__(self, directory, neg_size=99):
        ...
        nlp = stanza.Pipeline(use_gpu=True, processors='tokenize,lemma')  # use_gpu=True, processors='tokenize,lemma'
        vocab = set()
        title_words = []
        ...
        error_index = [550972, 685804, 725221, 896452]
        for u in tqdm.tqdm(range(len(self.users))):

            index = rating_groups.indices[u].tolist()
            try:
                interacted_movies = self.ratings['movie_idx'][index]
                timerank = self.ratings['timerank'][index]
            except KeyError:
