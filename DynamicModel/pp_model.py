...
from utils import time_trunc
from util.recorder import cur_time
from util.cmd_args import cmd_args
from util.consts import DEVICE, t_float
...
class HawkesProc(object):
    ...
    def survival(cur_user, cur_uembed, cur_item, cur_iembed,
                 user_embeds, user_ids, item_embeds, item_ids, t_end):
        u_dur = [time_trunc(t_end - cur_time.get_cur_time(u, cur_item)) for u in user_ids]
        # u_dur = [np.exp(-cur_time.get_cur_time(u, cur_item)) - np.exp(-t_end) for u in user_ids]
        u_dur = torch.tensor(u_dur, dtype=t_float).to(DEVICE)
        item_centric = HawkesProc.base_compatibility(user_embeds, cur_iembed) * u_dur

        i_dur = [time_trunc(t_end - cur_time.get_cur_time(cur_user, i)) for i in item_ids]
        # i_dur = [np.exp(-cur_time.get_cur_time(cur_user, i)) - np.exp(-t_end) for i in item_ids]
        i_dur = torch.tensor(i_dur, dtype=t_float).to(DEVICE)
        user_centric = HawkesProc.base_compatibility(cur_uembed, item_embeds) * i_dur
